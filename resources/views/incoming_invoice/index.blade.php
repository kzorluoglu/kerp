...
@section('content')
    ...
    <div class="row">
        <div class="col-md">
            <a href="{{route('incoming_invoice.select')}}"
               class="mb-2 mr-2 btn btn-primary">{{__('incoming_invoice.new_invoice')}}</a>
        </div>
        <div class="col-md">
                    ...
                    <table class="mb-0 table table-hover">
                        ...
                        @foreach($invoices as $invoice)
                            ...
                            <tr @if($invoice->paid) class="table-success" @endif>
                                <th scope="row">{{ $invoice->intern_invoice_number }}@if($invoice->paid) <span
                                ...
                                <td>{{ $invoice->city }}</td>
                                <td>
                                    <a href="{{ route('incoming_invoice.prepare', $invoice->id) }}"
                                       class="mb-2 mr-2 btn btn-light"><i class="metismenu-icon pe-7s-pen"></i></a>
                                    <a href="{{ route('incoming_invoice.paid', $invoice->id) }}"
                                       title="{{ __('invoice.paid')}}" class="mb-2 mr-2 btn btn-primary"><i
                                            class="metismenu-icon pe-7s-cash"></i></a>
