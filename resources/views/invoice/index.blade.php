                    ...
                    <table class="mb-0 table table-hover">
                        ...
                        @foreach($invoices as $invoice)
                            ...
                            <tr @if($invoice->paid) class="table-success" @endif>
                                <th scope="row">{{ $invoice->invoice_number }} @if($invoice->paid) <span
                                        class="badge badge-success badge-pill">{{ __('invoice.paid')}}</span> @endif
                                </th>
                                ...
                                <td>{{ $invoice->city }}</td>
                                <td>
                                    <a href="{{ route('invoice.prepare', $invoice->id) }}"
                                       class="mb-2 mr-2 btn btn-light"><i class="metismenu-icon pe-7s-pen"></i></a>
                                    <a href="{{ route('invoice.paid', $invoice->id) }}" title="{{ __('invoice.paid')}}"
                                       class="mb-2 mr-2 btn btn-primary"><i class="metismenu-icon pe-7s-cash"></i></a>
                                    <a class="mb-2 mr-2 btn btn-success" title="{!! __('invoice.pdf_preview') !!}"
                                       target="_blank" href="{{ route('invoice.pdf', $invoice->id ) }}"><i
                                            class="metismenu-icon pe-7s-file"></i></a>
                                    <a class="mb-2 mr-2 btn btn-success" title="{!! __('invoice.download') !!}"
                                       target="_blank" href="{{ route('invoice.download', $invoice->id ) }}"><i
